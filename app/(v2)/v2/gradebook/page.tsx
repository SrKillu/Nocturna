...
import { V2GradebookPage } from '@/components/v2/gradebook/v2-gradebook-page';
...
import { getMockGradebookV2 } from '@/lib/mocks/gradebook-v2';
import { canAccessGradebookV2 } from '@/lib/types/gradebook-v2';

export default async function GradebookV2Page() {
  let session;
  ...
  if (!canAccessGradebookV2(session.activeMembership.capabilities)) {
    return (
      <V2AccessDeniedState
  ...
  return (
    <V2GradebookPage
      gradebook={getMockGradebookV2(session.activeMembership.roleKey)}
    />
  );
}
