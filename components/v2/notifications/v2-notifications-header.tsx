...
export function V2NotificationsHeader() {
  return (
    <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <div className="flex flex-wrap items-center gap-2">
          <h1 className="text-2xl font-semibold tracking-tight">Notificaciones</h1>
          <Badge variant="outline">Fundación C22</Badge>
        </div>
        <p className="mt-1 text-sm text-muted-foreground">
          Consulta señales internas mock adaptadas al contexto de tu rol.
        </p>
      </div>
      <span className="flex items-center gap-2 rounded-md border bg-muted/25 px-3 py-2 text-sm font-medium text-muted-foreground">
        ...
        Solo lectura · sin envíos reales
      </span>
    </header>
  );
}
